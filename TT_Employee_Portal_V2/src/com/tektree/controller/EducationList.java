...
public class EducationList extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RequestDispatcher rd1 = request.getRequestDispatcher("EducationLoad.html");
		...
		try {
			...
			PreparedStatement pstmt=con.prepareStatement("select * from \"tektree\".\"employee_education\"");
...
//			int i=stmt.executeUpdate(("insert into \"tektree\".\"employee_education\"(employee_education_id,employee_id,degree,qualification,institute_name,passed_year,score_percentage)values(2,2,'"+Degree+"','"+Qualification+"','"+Institute_Name+"','"+Passed_Year+"','"+Score_Percentage+"')"));
			...
			while(rs.next())
			{
				...
				pw.println("<tr><td>"+rs.getString(3)+"</td><td>"+rs.getString(4)+"</td><td>"+rs.getString(5)+"</td><td>"+rs.getInt(6)+"</td><td>"+rs.getLong(7)+"</td><td><a href='UpdateAddress?Employee_Id="+rs.getInt("Employee_Id")+"' name='"+a+"'>edit</a></td></tr>");  
